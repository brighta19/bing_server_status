...
export const Scraper = {
    ...
    async getRecentEvents(): Promise<Event[]> {
        ...
        for (let i = 5; i < numOfEventsOnPage + 5; i++) {
            ...
            recentEvents.push(new Event({ id, url, date, type, status, description }));
        }

        return recentEvents;
    },

    async getEventDetails(eventURL: string): Promise<Event> {
        ...
        return new Event({ status, servicesImpacted, startTime, endTime, description, impactAnalysis, updates });
    }
}